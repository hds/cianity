...
Cianity will look for a file called `workflow.ci` (or `.workflow.ci` as a second choice) in the current directory and then going up towards the root of the file system.

In this case, no file needs to be specified. The rest of this guide will assume that a `workflow.ci` file is used.
...
As well as ensuring that the workflow can be [built](#build), a few other warning lints are also checked.
...
In order to check that the built pipeline is up to date with the ciane workflow, the `--check` flag can be used. It is recommended that this is used on CI to make sure that both checked in files match.
...
The `lsp` sub-command will also accept the `--stdio` flag which some IDEs pass unconditionally. This changes nothing as the cianity LSP only works over stdin/out.

## Help!

Use the `-h` flag or the `help` sub-command to see available sub-commands. The `-h` flag can be used with sub-commands to see all available flags.
